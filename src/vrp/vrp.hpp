...
class VRP {
  public:
    ...
    MPSolution solve(std::optional<size_t> subproblem_max_nb_solutions = std::nullopt,
      ...
      while (min_reduced_cost < -EPSILON) {
        ...
        (void)std::initializer_list<int>{([&]() {
          ...
          if (!solutions_boost.empty()) {
            if (!sols.empty()) {
              if (std::abs(solutions_boost[0].cost - sols[0].cost) > COST_COMPARISON_EPSILON) {
          ...
          if (!sols.empty()) {
            LOG_DEBUG("Solution RCSPP cost (algo ", algo_index, "): ", sols[0].cost, '\n');
          } else {
