...
class Graph {
  public:
    ...
    [[nodiscard]] const std::vector<size_t>& get_sink_node_ids() const { return sink_node_ids_; }
  ...
  private:
    ...
    virtual std::map<size_t, std::unique_ptr<Arc<ResourceType>>>::iterator delete_arc(
      ...
      in_arcs.erase(std::remove_if(in_arcs.begin(),
                                   in_arcs.end(),
      ...
      out_arcs.erase(std::remove_if(out_arcs.begin(),
                                    out_arcs.end(),
                                    [arc_id](Arc<ResourceType>* arc) { return arc->id == arc_id; }),
                     out_arcs.end());
