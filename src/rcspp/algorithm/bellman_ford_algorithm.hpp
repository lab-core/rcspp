    ...
    static Distance solve(const Graph<ResourceComposition<ResourceTypes...>>& graph_,
      ...
      for (const auto& [arc_id, arc] : graph_.get_arcs_by_id()) {
        ...
        const CostResourceType& origin_cost_resource =
          arc->origin->resource->template get_resource_component<CostResourceType>(cost_index);
        double origin_cost = origin_cost_resource.get_value();
        // expand the resource
        Resource<ResourceComposition<ResourceTypes...>> resource(*arc->destination->resource);
        ...
        arc_relaxations.emplace_back(arc->origin->id, arc->destination->id, cost - origin_cost);
      }
        ...
        for (const auto& relax : arc_relaxations) {
          if (forward &&
            ...
            modified = true;
                     ...
                     distance[relax.destination_id] + relax.weight < distance[relax.origin_id]) {
