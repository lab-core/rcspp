...
class PushingDominanceAlgorithmIterators : public DominanceAlgorithmIterators<ResourceType>,
  ...
  protected:
    LabelIteratorPair<ResourceType> next_label_iterator() override {
      ...
      while (this->current_unprocessed_labels_.empty()) {
        ...
        this->current_unprocessed_labels_ =
          std::move(this->unprocessed_labels_by_node_pos_.at(this->current_unprocessed_node_pos_));
      }
    ...
    [[nodiscard]] size_t number_of_labels() const override { return this->num_unprocessed_labels_; }
