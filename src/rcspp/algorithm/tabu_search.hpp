...
class TabuSearchAlgorithm : public BacktrackingDiveAlgorithm<ResourceType, LabelsType> {
    ...
    private:
        ...
        bool dive_to_sink() {
            ...
            // every 4096 steps.  The memory check matters: each extension
            // allocates a label, so a pathological dive is also an RSS
            // runaway (observed: a worker growing 4 -> 8 GB in 11 s on
            // setB-01 while max_memory_gb was 2.67 GB — only the dominance
            // loops checked the limit).
            size_t steps = 0;
