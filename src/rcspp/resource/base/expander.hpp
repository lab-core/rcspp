...
class Expander : public ResourceType {
  public:
    ...
    [[nodiscard]] auto create(const ResourceType& resource_base, const size_t arc_id) const
      ...
      auto new_expander =
        std::make_unique<Expander>(resource_base, expansion_function_->create(arc_id), arc_id);
    ...
    [[nodiscard]] auto create(const size_t arc_id) const
      -> std::unique_ptr<Expander<ResourceType>> {
      auto new_expander = std::make_unique<Expander>(expansion_function_->create(arc_id), arc_id);
...
class Expander<ResourceComposition<ResourceTypes...>>
  ...
  private:
    ...
    std::unique_ptr<ExpansionFunction<ResourceComposition<ResourceTypes...>>> expansion_function_;
