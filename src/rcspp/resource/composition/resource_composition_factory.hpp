...
class ResourceCompositionFactory : public ResourceFactory<ResourceComposition<ResourceTypes...>> {
  public:
    ...
    std::unique_ptr<ResourceComposition<ResourceTypes...>> make_resource_base(
      ...
      auto make_resource_function =
        [&](const auto& res_init_vec, auto& res_comp_vec, auto& res_fac_vec) {
          for (int i = 0; i < res_init_vec.size(); i++) {
            const auto& res_init = res_init_vec[i];

            const auto& res_comp = res_comp_vec.emplace_back(res_fac_vec[i]->make_resource_base());
            ...
            [&](auto&&... args_res_comp_vec) {
              std::apply(
                [&](auto&&... args_res_fac_vec) {
                  (make_resource_function(args_res_init_vec, args_res_comp_vec, args_res_fac_vec),
                   ...);
                },
        ...
        [&](auto&&... args_res_base_vec) {
          std::apply(
            [&](auto&&... args_res_fac_vec) {
              std::apply(
                [&](auto&&... args_res_comp_vec) {
                  (make_expander_function(args_res_base_vec, args_res_fac_vec, args_res_comp_vec),
                   ...);
                },
    ...
    ResourceFactory<ResourceType>& add_resource_factory(
      ...
      const auto& resource_factory_ref = std::get<ResourceTypeIndex>(resource_factory_components_)
    ...
    void update_expander(
      ...
      const std::tuple<std::vector<TypeTuples>...>& resource_initializer) {
      auto update_resource_function = [&](const auto& res_init_vec, const auto& res_comp_vec) {
        for (int i = 0; i < res_init_vec.size(); i++) {
          const auto& res_init = res_init_vec[i];
  ...
  private:
    ...
    const Resource<ResourceComposition<ResourceTypes...>>& update_resource_prototype() {
      auto& prototype_resource_components = this->resource_prototype_->get_resource_components();
