...
// The backward API's declarations, accessors and defaults, tested directly.
...
/// @brief backward_kind() and merge_rule() default to Unspecified on a class that declares
///        neither, which is what lets a bidirectional solve refuse an undeclared component.
...
/// @brief The composition back-dominance fan-out matches the forward one.
