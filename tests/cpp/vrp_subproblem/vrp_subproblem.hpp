...
class VRPSubproblem {
    ...
    public:
        ...
        // Test helper: solve and report the search work (extended and pooled labels) as well as
        // the result. The counters live on the algorithm, so this keeps it alive until read.
        struct RunMeasurement {
                ...
                // Lets callers tell a truncated solve from a genuinely cheaper one.
                AlgorithmStatus status = AlgorithmStatus::COMPLETE;
                // Bidirectional only; left at their defaults by every other algorithm.
        ...
        RunMeasurement solve_and_measure(const std::map<size_t, double>& dual_by_id,
            ...
            // Keep labels so the pool size reflects the peak rather than zero after release.
        ...
        // @param upper_bound Passed through to solve(). An infinite bound enables the joiner's
        //        incumbent cutoff, which makes the solution set order-dependent; tests comparing
        //        solution sets should pass a finite non-binding bound.
        template <template <typename, typename> class AlgorithmType = SimpleDominanceAlgorithm>
