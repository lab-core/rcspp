...
// The two size-cap merge-contract tests live in test_merge_contract.hpp, alongside the model they
// build: that rule is not an ng rule, and the pack is the same one, so keeping them there keeps
// this header about ng.
