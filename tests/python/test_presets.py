...
Each preset must match the hand-built model. Also pins the Python-only rule that the first
registered resource is a ``"real"`` cost resource, and the absence of an ng-path preset (its
components have no Python bindings).
"""
...
def test_preset_built_model_solves_bidirectionally():
    """A preset-built model is coherent backwards: the bidirectional solve finds a path."""
    rg = ResourceGraph()
...
def test_budget_preset_takes_a_uniform_capacity_only():
    """No per-node caps and no floor from Python.

    ``MinMaxFeasibilityFunction`` takes a single window from Python, and a floor on a threshold is
    never checked backward.
    """
    rg = ResourceGraph()
    presets.add_cost_resource(rg)
    with pytest.raises(TypeError):
        presets.add_budget_resource(rg, "int", 20, per_node={1: 5})
    with pytest.raises(TypeError):
        presets.add_budget_resource(rg, "int", 20, minimum=5)
...
def test_no_ng_path_preset_because_the_components_are_unbound():
    """No ng-path preset in Python: neither component it needs is exposed to Python."""
