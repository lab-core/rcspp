...
There are five complaints it can make, and they fall into two groups.  **Something was not
declared at all:**
...
a backward label carries a suffix. Four configurations do this today, and for each the fix is to
...
| `SizeFeasibilityFunction` | a non-zero **minimum** size anywhere | drop the floor, or enforce it after the solve; the cap is suffix-safe, the floor is a whole-path property |
...
In every one of the four, forward-only solves are unaffected.
...
| `its feasibility function forbids each node at itself … does not declare BackwardKind::NodeMirror` | The feasibility test asks *am I already in my own memory*, which needs the memory at a node to exclude that node in **both** directions.  Only a node-identity mirror gives that.  A `Mirror` container accumulates the arc's value, which is the same object going each way, so the backward label reaches a node already holding it and is rejected there — every time.  Use `NgPathExtensionFunction`, or `presets::add_elementary_resource` for an elementary path. |

(The messages are generated by `BidirectionalDominanceAlgorithm::describe_problem`; if you are
editing one, edit both.)

The first two pairing faults have the same usual cause, a **capacity written as an addition**:
`AdditionExtensionFunction` with `MinMaxFeasibilityFunction(0, capacity)` counts *up* from zero
while the backward seed says "start at the bound and count *down*".  That is fine
...
**disjoint** — which is what `IntersectionFeasibilityFunction` declares when it forbids each node at
itself, and what the ng-path relaxation means.
...
Anything else is refused at setup, because it fails in one of two directions and neither is
recoverable at the join:

- `forbidden(u) = {w}` for some other node `w` fails by **accepting**. A forward half can collect
  `w` before the meeting node while the suffix passes through `u`, and nothing checks it — the
  backward label never sees `w`, and disjointness does not either, because `w` is not on the
  suffix. The solve returns a path the model forbids, with a `complete` status.
- A forbidden set that binds nothing fails by **refusing**: the model permits every revisit, but
  disjointness still gates the join, so `bidirectional` answers a stricter question than `simple`.
...
- `NgPathExtensionFunction` reads them off the arc's *endpoints* and swaps which endpoint is "the
  node being left" per direction — `BackwardKind::NodeMirror`. The memory at `v` is the nodes
...
- `UnionExtensionFunction`, `IntersectionExtensionFunction` and `SubtractExtensionFunction`
  accumulate the arc's *value*, which is the same object both ways — `BackwardKind::Mirror`. With
...
`NgPathExtensionFunction` with an `IntersectionFeasibilityFunction` and leaves you to pass
`forbidden_by_node[v] = {v}`.
...
backwards for the reason the next section gives.
